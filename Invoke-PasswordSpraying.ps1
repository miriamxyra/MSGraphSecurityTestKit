...
function Invoke-PasswordSpraying {
    ...
    $UsersAndPasswords = :main ForEach ($Password in $Passwords) {
        ...
        ForEach ($Username in $Usernames) {
            ...
            try {
                $WebRequest = Invoke-WebRequest $Url/common/oauth2/token -Method Post -Headers $HeaderParameters -Body $BodyParameters -ErrorVariable WebrequestError -UserAgent "MSGraphSecurityTestKit"
            }
            catch {
